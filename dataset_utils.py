...
class DatasetDownloader:
    ...
    @staticmethod
    def download_dataset(dataset_name: str, path_to_data: str = "./data"):
        if dataset_name == "cifar10":
    ...
    @staticmethod
    def download_cifar10(path_to_data: str = "./data"):
    ...
    @staticmethod
    def download_mnist(path_to_data: str = "./data"):
        # download dataset and load train set
        ...
        # fuse all data splits into a single "training.pt"
        data_loc = Path(path_to_data) / "mnist-10-batches-py"
        training_data = data_loc / "training.pt"
    ...
    def get_dataset(path_to_data: Path, cid: str, partition: str, dataset: str):
        ...
        return TorchVision_FL(
            path_to_data,
            transform=DatasetDownloader.get_transformation(dataset),
        )
    ...
    def get_dataloader(
        ...
        dataset = DatasetDownloader.get_dataset(
            Path(path_to_data), cid, partition, dataset
        )
    ...
    def do_fl_partitioning(
        path_to_dataset, pool_size, alpha, num_classes, val_ratio=0.0
    ):
