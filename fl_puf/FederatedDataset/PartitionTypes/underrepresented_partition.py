...
class UnderrepresentedPartition:
    def do_partitioning(
        ...
        represented_indexes = []
        represented_labels = []
        for index, sens_feature, label in zip(indexes, sensitive_features, current_labels):
        ...
        splitted_indexes_underrepresented += NonIIDPartition.do_partitioning_with_indexes(indexes=torch.tensor(underrepresented_indexes), labels=torch.tensor(underrepresented_labels), num_partitions=underrepresented_nodes, alpha=alpha)
        splitted_indexes_represented += NonIIDPartition.do_partitioning_with_indexes(indexes=torch.tensor(represented_indexes), labels=torch.tensor(represented_labels), num_partitions=represented_nodes, alpha=alpha)

        splitted_indexes = []
        ratio_splitted = int(10*ratio)
        for index in range(len(splitted_indexes_represented)+len(splitted_indexes_underrepresented)):
