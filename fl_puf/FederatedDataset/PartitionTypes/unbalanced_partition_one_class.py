...
class UnbalancedPartitionOneClass:
    def do_partitioning(
        ...
        splitted_indexes += IIDPartition.do_iid_partitioning_with_indexes(indexes=indexes,  num_partitions=num_partitions)
        ...
        for index_list in splitted_indexes:
            ...
            if num_nodes_with_2_combinations > 0:
                for item in index_list:
                    ...
                    if (indexes_and_labels[item] == 1 and indexes_and_sensitive_features[item] == 1) or (indexes_and_labels[item] == 0 and indexes_and_sensitive_features[item] == 1):
                        continue
                    else:
