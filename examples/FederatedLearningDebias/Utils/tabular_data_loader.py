...
def load_compas(dataset_path):
    def load_preproc_data_compas2(df2, protected_attributes=None):
        def custom_preprocessing(df2):
            ...
            df2 = df2.loc[ix, :]
            df2["length_of_stay"] = (pd.to_datetime(df2["c_jail_out"]) - pd.to_datetime(df2["c_jail_in"])).apply(
                lambda x: x.days
            )

            # Restrict races to African-American and Caucasian
            df2cut = df2.loc[~df2["race"].isin(["Native American", "Hispanic", "Asian", "Other"]), :]
            ...
            df2cutQ["priors_count"] = df2cutQ["priors_count"].apply(lambda x: quantizePrior(x))
            df2cutQ["length_of_stay"] = df2cutQ["length_of_stay"].apply(lambda x: quantizeLOS(x))
            df2cutQ["score_text"] = df2cutQ["score_text"].apply(lambda x: quantizeScore(x))
...
def load_bank_marketing():
    bank_marketing_df = pd.read_csv("data/Tabular/bank_marketing/bank-full.csv", sep=";")
    # count number of 'married' in marital column
    print(bank_marketing_df["marital"].value_counts())

    bank_marketing_df["marital_binary"] = np.where(bank_marketing_df["marital"] == "married", 1, 0)
    bank_marketing_df["age_binary"] = np.where((bank_marketing_df["age"] > 25) & (bank_marketing_df["age"] < 60), 1, 0)
...
def load_credit_card():
    ...
    credit_card_df["education_binary"] = np.where(credit_card_df["EDUCATION"] == 2, 1, 0)
...
def load_communities_and_crime():
    communities_and_crime_df = pd.read_csv("data/Tabular/crimedata.csv", encoding="latin")
    communities_and_crime_df = communities_and_crime_df.dropna(axis=1, how="all")

    communities_and_crime_df = communities_and_crime_df[communities_and_crime_df["ViolentCrimesPerPop"] != "?"]
    communities_and_crime_df = communities_and_crime_df[communities_and_crime_df["ViolentCrimesPerPop"] != "y"]

    communities_and_crime_df["ViolentCrimesPerPop"] = communities_and_crime_df["ViolentCrimesPerPop"].astype(float)
    communities_and_crime_df["is_violent"] = np.where(communities_and_crime_df["ViolentCrimesPerPop"] > 0.7, 1, 0)
    communities_and_crime_df["is_black"] = np.where(communities_and_crime_df["racepctblack"] > 0.06, 1, 0)
    ...
    assert communities_and_crime_df.isnull().sum().sum() == 0, "Encountered missing values!"
...
def load_diabetes():
    ...
    diabetes_df = diabetes_df.drop(diabetes_df[diabetes_df["gender"] == "Unknown/Invalid"].index)
    ...
    diabetes_df = diabetes_df.dropna(subset=["race", "diag_1", "diag_2", "diag_3"], how="all")
...
# def load_dutch_unprivileged(dataset_path):
#     # data = arff.loadarff(dataset_path + "dutch_census.arff")
#     # dutch_df = pd.DataFrame(data[0]).astype("int32")
#     dutch_df = pd.read_csv(dataset_path + "dutch_unprivileged.csv")
...
def load_dutch_unprivileged(dataset_path):
    ...
    dutch_df = pd.read_csv(dataset_path + "dutch_census_edu_level.csv")
...
def load_dutch_unprivileged_poisoned(dataset_path):
    # data = arff.loadarff(dataset_path + "dutch_census.arff")
    # dutch_df = pd.DataFrame(data[0]).astype("int32")
    # dutch_df = pd.read_csv(dataset_path + "dutch_census.csv")
    dutch_df = pd.read_csv(dataset_path + "dutch_census_edu_level_poisoned.csv")
...
def get_tabular_numpy_dataset(dataset_name, num_sensitive_features, dataset_path=None):
    ...
    elif dataset_name == "dutch" or dataset_name == "dutch_valerio":
        tmp = load_dutch(dataset_path=dataset_path)
    elif dataset_name == "dutch_unprivileged":
        tmp = load_dutch_unprivileged(dataset_path=dataset_path)
        _X, _Z, _y = dataset_to_numpy_mod(*tmp, num_sensitive_features=num_sensitive_features)
        return _X, _Z, _y
    elif dataset_name == "dutch_unprivileged_poisoned":
        tmp = load_dutch_unprivileged_poisoned(dataset_path=dataset_path)
        _X, _Z, _y = dataset_to_numpy_mod(*tmp, num_sensitive_features=num_sensitive_features)
        return _X, _Z, _y
...
def prepare_tabular_data(
    ...
    if dataset_name == "income" or dataset_name == "income_valerio":
        for client_name in range(num_nodes):
            os.system(f"rm -rf {dataset_path}/{splitted_data_dir}/{client_name}/train.pt")

            # open numpy arrays
            X = np.load(f"{dataset_path}/{splitted_data_dir}/{client_name}/income_dataframes_{client_name}.npy")
            Y = np.load(f"{dataset_path}/{splitted_data_dir}/{client_name}/income_labels_{client_name}.npy")
            Z = np.load(f"{dataset_path}/{splitted_data_dir}/{client_name}/income_groups_{client_name}.npy")
        ...
        return fed_dir, None
    elif dataset_name == "continual_income" or dataset_name == "continual_traffic" or dataset_name == "continual_dutch":
        for client_name in range(num_nodes):
            os.system(f"rm -rf {dataset_path}/{splitted_data_dir}/{client_name}/train.pt")

            # open numpy arrays
            X = np.load(f"{dataset_path}/{splitted_data_dir}/{client_name}/income_dataframes_0.npy")
            Y = np.load(f"{dataset_path}/{splitted_data_dir}/{client_name}/income_labels_0.npy")
            Z = np.load(f"{dataset_path}/{splitted_data_dir}/{client_name}/income_groups_0.npy")
            custom_dataset = TabularDataset(
                x=np.hstack((X, np.ones((X.shape[0], 1)))).astype(np.float32),
                z=[item.item() for item in Z],  # .astype(np.float32),
                y=[item.item() for item in Y],  # .astype(np.float32),
