...
class NonIIDPartition:
    def do_partitioning(
        ...
        num_sensitive_attributes = len(set([item.item() for item in sensitive_attributes]))
        idx = torch.tensor(list(range(len(sensitive_attributes))))

        index_per_sensitive_attribute = {}
        for index, feature in zip(idx, sensitive_attributes):
            if feature.item() not in index_per_sensitive_attribute:
                index_per_sensitive_attribute[feature.item()] = []
            index_per_sensitive_attribute[feature.item()].append(index.item())

        # in list labels we have the labels of this dataset
        list_sensitive_attributes = {item.item() for item in sensitive_attributes}

        to_be_sampled = []
        # create the distribution for each class
        for label in list_sensitive_attributes:
            ...
            selected_sensitive_attributes = sensitive_attributes[sensitive_attributes == label]
            tmp_to_be_sampled = np.random.choice(num_partitions, len(selected_sensitive_attributes), p=distribution[0])
        ...
        partitions_index = {f"node_{partition_name}": [] for partition_name in range(0, num_partitions)}
        for class_index, distribution_samples in zip(list_sensitive_attributes, to_be_sampled):
            for partition_name, samples in distribution_samples.items():
                partitions_index[f"node_{partition_name}"] += index_per_label[class_index][:samples]
        ...
        partitions_sensitive_attributes = {
            cluster: [item.item() for item in sensitive_attributes[samples]]
            for cluster, samples in partitions_index.items()
        }

        partitions_data = {cluster: data[samples] for cluster, indexes in partitions_index.items()}

        return partitions_index, partitions_sensitive_attributes

    # def do_partitioning_with_indexes(
    #     indexes: np.array,
    #     labels: np.array,
    #     num_partitions: int,
    #     alpha=1000000,
