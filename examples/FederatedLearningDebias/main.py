...
from client import FlowerClientDisparity
from client_error_rate import FlowerClientErrorRate
...
parser.add_argument("--second_target", type=float, default=None)
...
parser.add_argument("--global_computation", type=bool, default=False)


parser.add_argument("--shift", type=bool, default=False)
parser.add_argument("--train_csv_shift", type=str, default="")
parser.add_argument("--number_of_samples_per_node_shift", type=int, default=None)  # maximum number of samples per node
parser.add_argument(
    "--ratio_unfairness_shift", type=float, default=None, nargs="+"
)  # percentage of samples removed from group_to_reduce on the unfair nodes
parser.add_argument(
    "--ratio_unfair_nodes_shift", type=float, default=None
)  # percentage of nodes that will be unbalanced (unfair nodes)
...
if __name__ == "__main__":
    ...
    train_parameters = TrainParameters(
        ...
        shift=args.switch_dataset,
        global_computation=args.global_computation,
    ...
    if args.tabular_data:
        ...
        fed_dir, _ = prepare_tabular_data(
            ...
            opposite_ratio_unfairness=tuple(args.opposite_ratio_unfairness) if args.opposite_ratio_unfairness else None,
        ...
        print("------> Fed Dir: ", fed_dir)
        if args.shift:
            train_set_shift, test_set_shift = DatasetUtils.download_dataset(
                dataset_name,
                train_csv=args.train_csv_shift,
                debug=False,
                base_path=args.dataset_path,
            )

            train_path_shift = Utils.prepare_dataset_for_FL(
                dataset=train_set_shift,
                dataset_name=dataset_name,
                base_path=args.base_path,
            )
            # Partitioning the training dataset
            fed_dir_shift = Utils.do_fl_partitioning(
                train_path_shift,
                pool_size=pool_size,
                num_classes=2,
                val_ratio=0,
                ...
                group_to_reduce=tuple(args.group_to_reduce) if args.group_to_reduce else None,
                group_to_increment=tuple(args.group_to_increment) if args.group_to_increment else None,
                number_of_samples_per_node=args.number_of_samples_per_node_shift,
                ratio_unfair_nodes=args.ratio_unfair_nodes_shift if args.ratio_unfair_nodes_shift else None,
                ratio_unfairness=tuple(args.ratio_unfairness_shift) if args.ratio_unfairness_shift else None,
                one_group_nodes=args.one_group_nodes,
                splitted_data_dir=args.splitted_data_dir,
                shift=True,
            )
