...
class FlowerClient(NumPyClient):
    def __init__(
        ...
        ) if self.preferences.private_training else 0.0

        model_gc, optimizer_gc, criterion_gc, train_loader_gc = self.privacy_engine.make_private(
        ...
        self.model = PUFFLEModel(
            ...
            momentum=self.preferences.momentum if self.preferences.momentum is not None else None,
            alpha=self.preferences.alpha if self.preferences.alpha is not None else None,
            weight_decay_alpha=self.preferences.weight_decay_alpha if self.preferences.weight_decay_alpha is not None else None,
        )
    ...
    def fit(self, parameters: NDArrays, config: dict[str, Scalar]) -> tuple[NDArrays, int, dict[str, Any]]:
        ...
        for _ in range(self.preferences.num_epochs):
            result_dict = self.model.train(train_loader=self.trainloader, epochs=self.preferences.num_epochs)
