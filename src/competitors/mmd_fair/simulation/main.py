# ABOUTME: Entry point for MMD-Fair FL simulation using Flower.
# ABOUTME: Mirrors PUFFLE's main.py but swaps the strategy for MMDFairFedAvg.
...
def server_fn(context: Context) -> ServerAppComponents:
    ...
    strategy = MMDFairFedAvg(
        fraction_fit=preferences.sampled_training_nodes_per_round or 0.3,
        fraction_evaluate=(
            preferences.sampled_validation_nodes_per_round
            if preferences.sweep
            else preferences.sampled_test_nodes_per_round
        )
        or 0.0,
...
def main():
    ...
    # Fairness partitioner args (matching PUFFLE)
    ...
    if args.node_shuffle_seed is None:
        args.node_shuffle_seed = int(str(time.time()).split(".")[1]) * args.seed
