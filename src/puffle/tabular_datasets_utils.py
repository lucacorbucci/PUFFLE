...
def load_kdd():
    ...
    kdd_census_df = pd.read_csv("data/Tabular/kdd/KDD-census-income.data", names=kdd_columns)
...
def generate_clients_biased_data(x, y, z, M, do_iid_split, clients_balance_factor, priv_balance_factor):
    """Args:
    ...
    assert 0.0 <= clients_balance_factor <= 1.0 and 0.5 <= priv_balance_factor <= 1.0, "Invalid privileged fractions!"
    N = x.shape[0]
    ...
    assert np.all(len(np.unique(z)) == 2), f"Sensitive features not properly 1-hot encoded! Got uniques: {np.unique(z)}"
    assert z.shape[1] == 2, "Currently only single 1-hot encoded sensitive feature supported!"
    # x_priv = x[z==1]
    ...
    M_unpriv = M - M_priv
    assert M_priv > 0 and M_unpriv > 0, (
    ...
    unpriv_unpriv_client_size = int((N_unpriv - M_priv * priv_unpriv_client_size) / M_unpriv)
    assert (
    ...
    for i in range(M_priv):
        client_x = np.vstack((x_priv[:priv_priv_client_size], x_unpriv[:priv_unpriv_client_size]))
        x_priv = x_priv[priv_priv_client_size:]
        x_unpriv = x_unpriv[priv_unpriv_client_size:]
        client_y = np.concatenate((y_priv[:priv_priv_client_size], y_priv[:priv_unpriv_client_size]))
    ...
    for i in range(M_unpriv):
        client_x = np.vstack((x_priv[:unpriv_priv_client_size], x_unpriv[:unpriv_unpriv_client_size]))
        x_priv = x_priv[unpriv_priv_client_size:]
        x_unpriv = x_unpriv[unpriv_unpriv_client_size:]
        client_y = np.concatenate((y_priv[:unpriv_priv_client_size], y_unpriv[:unpriv_unpriv_client_size]))
