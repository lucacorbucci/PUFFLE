...
class BalancedAndUnbalanced:
    def do_partitioning(
        ...
        for label, sens_feature, index in zip(labels, sensitive_features, range(len(labels))):
        ...
        min_instances = min([len(index_list) for index_list in labels_and_sensitive.values()])
        print(
            "LENGHT: ",
        ...
        for key, index_list in labels_and_sensitive.items():
            ...
            while len(index_list) > min_instances:
                removed_instances[key].append(index_list.pop(random.randrange(len(index_list))))
        ...
        balanced_indexes = np.array(list(itertools.chain.from_iterable(labels_and_sensitive.values())))
        ...
        ratio_to_be_unbalanced = int(len(balanced_partition) * percentage_unbalanced_nodes)
        ...
        for indexes in to_be_unbalanced:
            ...
            indexes_to_remove = indexes_to_remove[: int(len(indexes_to_remove) * unbalanced_ratio)]
            removed_indexes = len(indexes_to_remove)
            indexes = np.array([index for index in indexes if index not in indexes_to_remove])
        ...
        print(f"Len Balanced {len(balanced_partition)} Len Unbalanced {len(new_to_be_unbalanced)}")
        ...
        return balanced_partition + new_to_be_unbalanced, [0] * len(balanced_partition) + [1] * len(
            new_to_be_unbalanced
        )
