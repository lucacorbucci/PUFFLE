...
class NonIIDPartitionNodesWithSensitiveFeature:
    def do_partitioning(
        ...
        for partition in range(num_partitions):
            distribution = np.random.dirichlet(len(labels_and_sensitive_feature) * [alpha], size=1)
        ...
        for (label, sensitive_feature), distribution in zip(labels_and_sensitive_feature, transposed_distributions):
        ...
        partitions_index = {f"node_{cluster_name}": [] for cluster_name in range(0, num_partitions)}
        total_samples = 0
