...
class MajorityMinorityPartition:
    ...
    def do_partitioning(
        ...
        labels_list = sorted({item.item() if isinstance(item, torch.Tensor) else item for item in labels})
        ...
        if num_labels >= num_partitions:
            ...
            n_partitions_minority_classes = int((num_partitions - 1) / num_nodes_split_minority)
            ...
            minority_classes_per_partition = int(
                max(
                    ((num_partitions * majority_classes_per_partition) * n_partitions_minority_classes)
                    / num_partitions,
                    1,
                )
            )
            majority_classes = {
                ...
                for item in labels_list[: majority_classes_per_partition * num_partitions]
            }
            ...
            n_partitions_minority_classes = int(
                (num_partitions - majority_classes_per_partition) / num_nodes_split_minority
            )
            percentage_majority_class_splitted = percentage_majority_class / majority_classes_per_partition
            percentage_minority_per_partition = round(
        ...
        minority_classes_to_be_assigned = MajorityMinorityPartition.get_minority_classes_to_be_assigned(
            majority_classes=majority_classes,
    ...
    def assign_majority_classes(
        ...
        percentage_majority_assigned_to_each_partition = {partition_id: {} for partition_id in partition_names}
        assigned_majority_classes = set()
        counter_partitions = {partition_name: 0 for partition_name in partition_names}
        sorted_values = [item[0] for item in sorted(counter_partitions.items(), key=lambda x: x[1])]
        added = False
        ...
        while majority_classes:
            ...
            for partition_id in sorted_values:
                if majority_classes:
                    ...
                    majority_classes[majority_class] -= percentage_majority_per_partition
                    if majority_classes[majority_class] <= 0:
                        majority_classes.pop(majority_class)

                    percentage_majority_assigned_to_each_partition[partition_id][majority_class] = (
                        percentage_majority_per_partition
                    )
    ...
    def assign_minority_classes(
        ...
        percentage_minority_assigned_to_each_partition = {partition_id: {} for partition_id in partition_names}
        while minority_classes_to_be_assigned:
            ...
            for partition_id in sorted_values:
                if (
                    len(
                        percentage_minority_assigned_to_each_partition.get(partition_id, []),
                    )
                    < n_partitions_minority_classes
                    and minority_class_name not in percentage_majority_assigned_to_each_partition[partition_id].keys()
                    and minority_class_name not in percentage_minority_assigned_to_each_partition[partition_id].keys()
                ):
                    percentage_minority_assigned_to_each_partition[partition_id][minority_class_name] = (
    ...
    def get_minority_classes_to_be_assigned(
        ...
        for item in majority_classes:
            for _ in range(n_partitions_minority_classes):
                minority_classes_to_be_assigned.append((item, percentage_minority_per_partition))
        ...
        if len(unassigned_labels) > 0:
            for partition_name in final_percentage_assignment:
                for unassigned_label in unassigned_labels:
                    final_percentage_assignment[partition_name][unassigned_label] = 1 / num_partitions
    ...
    def split_labels_and_indices(
        ...
        labels = [item.item() if isinstance(item, torch.Tensor) else item for item in dataset.targets]
        ...
        splitted_labels = {
            partition_name: [
                item.item() if isinstance(item, torch.Tensor) else item for item in dataset.targets[indexes]
            ]
            for partition_name, indexes in splitted_indexes.items()
        }

        splitted_data = {
            partition_name: np.array(data)[indexes] for partition_name, indexes in splitted_indexes.items()
        }
