...
from FederatedDataset.PartitionTypes.non_iid_partition_nodes import (
...
class FederatedDataset:
    def generate_partitioned_dataset(
        ...
        split_type_clusters = config.data_split_config.split_type_clusters if config else split_type_clusters
        split_type_nodes = config.data_split_config.split_type_nodes if config else split_type_nodes
        ...
        if cluster_splits_train:
            partitions_train = FederatedDataset.create_partitioned_dataset_with_clusters(
            ...
            for node_name, labels in splitted_labels_cluster.items():
                ...
                if dataset_with_csv:
                    train_partition = MyDatasetWithCSV(
                        ...
                        transform=transform,
                        sensitive_features=torch.tensor(dataset.sensitive_features)[indexes]
                        if hasattr(dataset, "sensitive_features")
                        else None,
                    )
