...
class FlowerClientDisparity(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config, average_probabilities=None):
        ...
        if current_fl_round > 20 and self.train_parameters.second_target is not None:
            self.train_parameters.target = self.train_parameters.second_target
        ...
        print(
            f"Loaded average probabilities on client {self.cid}: ",
        ...
        if (
            self.train_parameters.shift is not None
            and self.train_parameters.shift <= current_fl_round
            and self.dataset_name == "celeba"
        ):
            partition = "train_1"
        else:
            partition = "train"
        ...
        if self.train_parameters.epsilon_lambda is not None:
            ...
            iterations = (
                self.sampling_frequency * self.train_parameters.epochs * len(train_loader) * 2
                if self.train_parameters.global_computation
                else 1  # we add the two here because in the binary case
            )
        ...
        if self.train_parameters.global_computation:
            ...
            max_disparity_train_before_local_epoch = max_disparity_train_before_local_epoch.cpu().numpy()
        ...
        if sum_counters and global_counters:
            ...
            max_disparity_train_before_local_epoch = fairness_violation.item()
        ...
        if not first_round and self.train_parameters.target and is_tunable:
            ...
            #     )
            # )
            max_disparity_train_before_local_epoch = max_disparity_train_before_local_epoch
            ...
            print(
                f"Client {self.cid} FL round {current_fl_round} Max Disparity: {max_disparity_train_before_local_epoch}, Lambda: {self.train_parameters.regularization_lambda}"
            )
        ...
        all_losses = []
        all_losses_or = []
        all_losses_with_regularization = []
        history_lambda = []
        history_accuracy = []
        history_disparity = []
        for epoch in range(0, self.train_parameters.epochs):
            ...
            history_accuracy.extend(metrics["history_accuracy"])
            history_disparity.extend(metrics["history_disparity"])
            all_metrics.append(metrics)
            all_losses.append(metrics["Train Loss"])
            all_losses_or.extend(metrics["Train Loss_or"])
            all_losses_with_regularization.extend(metrics["Train Loss + Regularizaion_or"])
        ...
        if self.train_parameters.epsilon_statistics is not None:
            ...
            sampling_ratio = 1
            iterations = self.sampling_frequency * len(combinations)
            # we multiply by len of the combination because every time we send len(combination) values
            # if we are in a binary case we only have two combinations.
        ...
        return (
            ...
            len(train_loader.dataset),
            {
                ...
                "all_losses_or": all_losses_or,
                "all_losses_with": all_losses_with_regularization,
                ...
                "delta": final_delta,
                "Average Probabilities": probabilities,
                "cid": self.cid,
                ...
                "current_target": self.train_parameters.target,
            },
        )
    ...
    def compute_starting_lambda_with_disparity(self, disparity_training: float):
        ...
        if delta > 0:
            ...
            # delta = disparity_training - self.train_parameters.target
            # delta = float(torch.tensor(delta).to("cpu"))
            # starting_lambda = delta / self.train_parameters.target
            # print("Starting Lambda: ", starting_lambda)
            # return starting_lambda
