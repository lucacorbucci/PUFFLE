...
class Utils:
    ...
    def setup_wandb(args, train_parameters):
        ...
        os.environ["WANDB_CACHE_DIR"] = "/raid/lcorbucci/wandb_tmp"
    ...
    def get_dataset(path_to_data: Path, cid: str, partition: str, dataset: str):
        ...
        if dataset == "dutch" or dataset == "dutch_unprivileged":
            return torch.load(path_to_data)
        elif (
            dataset == "income"
            or dataset == "continual_income"
            or dataset == "continual_traffic"
            or dataset == "continual_dutch"
        ):
            return torch.load(path_to_data)
        else:
    ...
    def do_fl_partitioning(
        ...
        if splits_dir.exists() and partition == "train" and not shift:
        ...
        for p in range(pool_size):
            ...
            if partition == "train":
                if shift:
                    file_name = "train_1.pt"
                else:
                    file_name = "train.pt"
            else:
                file_name = "test.pt"
            with open(
                splits_dir / str(p) / (file_name),
                "wb",
            ) as f:
        ...
        Utils.plot_bar_plot(
            title="Distribution Disparities" if not shift else "Distribution Disparities Shift",
        ...
        Utils.plot_distributions(
            title="Distribution of the nodes" if not shift else "Distribution of the nodes Shift",
