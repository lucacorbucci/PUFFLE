...
class FlowerClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config, average_probabilities=None):
        ...
        max_disparity_train_before_local_epoch = EqualizedOddsLoss().violation_with_dataset(
            model=private_net,
        ...
        if not first_round and self.train_parameters.target and is_tunable:
            if max_disparity_train_before_local_epoch is None:
                self.train_parameters.regularization_lambda = 0
            else:
        ...
        (probabilities, counters) = EqualizedOddsLoss.compute_probabilities(
        ...
        if self.train_parameters.epsilon_statistics is not None:
            sampling_ratio = 1
            iterations = self.sampling_frequency * (
                2 if self.train_parameters.metric == "disparity" else 8
        ...
        return (
            ...
            len(train_loader.dataset),
            {
                ...
                if all_metrics[-1]["Max Disparity Train"] != None
                else 0,
    ...
    def evaluate(self, parameters, config):
        ...
        (
            probabilities,
            counters,
        ) = EqualizedOddsLoss.compute_probabilities(
            predictions=predictions,
            sensitive_attribute_list=sensitive_attributes,
            device=self.train_parameters.device,
            possible_sensitive_attributes=possible_sensitive_attributes,
        ...
        if self.train_parameters.sweep:
            metrics = {
                ...
                "max_disparity_validation": float(max_disparity) if max_disparity != None else 0,
            ...
            metrics = {
                ...
                "max_disparity_test": float(max_disparity) if max_disparity != None else 0,
                "test_loss": test_loss,
                "probabilities": probabilities,
                "cid": self.cid,
                "counters": counters,
