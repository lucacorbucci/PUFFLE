...
class AveragePredictiveValueDifferenceLoss(nn.Module):
    ...
    def forward(
        ...
        for sensitive_value in possible_sensitive_attributes:
            ...
            tp_sv = torch.sum(
                softmax_[
                    (predictions_argmax == 1) & (sensitive_attribute_list == sensitive_value) & (true_targets == 1)
                ][:, 1]
            )
            tp_opposite_sv = torch.sum(
                softmax_[
                    (predictions_argmax == 1)
                    & (sensitive_attribute_list == opposite_sensitive_value)
                    & (true_targets == 1)
                ][:, 1]
            )
            fp_sv = torch.sum(
                softmax_[
                    (predictions_argmax == 1) & (sensitive_attribute_list == sensitive_value) & (true_targets == 0)
                ][:, 1]
            )
            fp_opposite_sv = torch.sum(
                softmax_[
                    (predictions_argmax == 1)
                    & (sensitive_attribute_list == opposite_sensitive_value)
                    & (true_targets == 0)
            ...
            fn_sv = torch.sum(
                softmax_[
                    (predictions_argmax == 0) & (sensitive_attribute_list == sensitive_value) & (true_targets == 1)
                ][:, 0]
            )
            fn_opposite_sv = torch.sum(
                softmax_[
                    (predictions_argmax == 0)
                    & (sensitive_attribute_list == opposite_sensitive_value)
                    & (true_targets == 1)
                ][:, 0]
            )
            tn_sv = torch.sum(
                softmax_[
                    (predictions_argmax == 0) & (sensitive_attribute_list == sensitive_value) & (true_targets == 0)
                ][:, 0]
            )
            tn_opposite_sv = torch.sum(
                softmax_[
                    (predictions_argmax == 0)
                    & (sensitive_attribute_list == opposite_sensitive_value)
                    & (true_targets == 0)
            ...
            violation_term = 0.5 * (
                ((tp_sv / (tp_sv + fp_sv)) - (tp_opposite_sv / (tp_opposite_sv + fp_opposite_sv)))
                + ((fn_sv / (fn_sv + tn_sv)) - (fn_opposite_sv / (fn_opposite_sv + tn_opposite_sv)))
            )

            fairness_violations.append(abs(violation_term))
    ...
    def count_true_positive(self, y_true, y_pred, group, sensitive_value):
        true_positive = 0
    ...
    def count_false_positive(self, y_true, y_pred, group, sensitive_value):
        false_positive = 0
        for i in range(len(y_true)):
    ...
    def count_true_negative(self, y_true, y_pred, group, sensitive_value):
        true_negative = 0
        for i in range(len(y_true)):
    ...
    def count_false_negative(self, y_true, y_pred, group, sensitive_value):
        false_negative = 0
        for i in range(len(y_true)):
    ...
    def compute_PPV(self, y_true, y_pred, group, sensitive_value):
        true_positive = self.count_true_positive(y_true, y_pred, group, sensitive_value)
        false_positive = self.count_false_positive(y_true, y_pred, group, sensitive_value)
        return true_positive / (true_positive + false_positive)

    def compute_for(self, y_true, y_pred, group, sensitive_value):
        true_negative = self.count_true_negative(y_true, y_pred, group, sensitive_value)
        false_negative = self.count_false_negative(y_true, y_pred, group, sensitive_value)
