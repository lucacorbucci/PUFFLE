...
from ..Regularization.ErrorRateRegularizationLoss import ErrorRateRegularizationLoss
...
class Learning:
    ...
    def train_private_model(
        ...
        elif train_parameters.metric == "error_rate":
            criterion_regularization = ErrorRateRegularizationLoss()
        losses = []
        losses_with_regularization = []
        total_correct = 0
        total = 0
        velocity = train_parameters.velocity if train_parameters.velocity else 0
        history_lambda = []
        history_disparity = []
        history_accuracy = []
        ...
        with BatchMemoryManager(
            ...
            for batch_number, (data, sensitive_feature, target) in enumerate(memory_safe_data_loader, 0):
                regularization_term = None
                ...
                if train_parameters.regularization and model_regularization:
                    ...
                    if fairness_violation is not None and fairness_violation > 0:
                        ...
                        # regularization_term = fairness_violation
                        try:
                            regularization_term.backward()
                        except Exception as e:
                            print(f"Error in the backward pass of the regularization term: {e}")
                            print(traceback.format_exc())
                            continue

                outputs = model(data)

                if not train_parameters.regularization:
                    # adding this for logging purposes
                    fairness_violation = Learning.compute_regularization_term(
                        data=data,
                        targets=target,
                        sensitive_feature=sensitive_feature,
                    ...
                    history_disparity.append(fairness_violation)
                    fairness_violation = None
                else:
                    history_lambda.append(train_parameters.regularization_lambda)

                history_accuracy.append((outputs.argmax(dim=1) == target).sum().item() / target.size(0))
                ...
                loss.backward()
                print(
                    f"Using Model Loss {loss} - Classic Loss {classic_loss} - Unfairness Loss {regularization_term} - Fairness Violation {fairness_violation} - Lambda {train_parameters.regularization_lambda} ",
                )
                ...
                if fairness_violation is not None and train_parameters.target and is_tunable:
                    # this is new: we want to test if we can update the lambda
                    # computing the fairness after the update of the model
                    model.eval()
                    output_regularization = model(data)
                    fairness_violation = None
                    with open(f"{train_parameters.fed_dir}/metadata.json", "r") as infile:
                        json_file = json.load(infile)

                    if train_parameters.global_computation:
                        if train_parameters.metric == "disparity":
                            # If we want to take into account the global disparity computation
                            # when computing the local disparity then we have the
                            # global_computation parameter set to True.
                            (
                                fairness_violation,
                                global_counters,
                                ...
                                average_probabilities=average_probabilities,
                                batch=(epoch + 1) * batch_number,
                                global_computation=train_parameters.global_computation,
                                json_file=json_file,
                            )
                            # we introduce the necessary noise to the global_counters

                            for combination in json_file["combinations"]:
                                if sigma_update_lambda:
                                    ...
                                    noise = 0
                                # We introduce noise based on the sigma that we computed
                                print("Noise introduced: ", noise)
                                if combination in global_counters:
                                    global_counters[combination] += noise
                                    if global_counters[combination] < 0:
                                        global_counters[combination] = 0

                            # Compute the missing counters
                            for non_existing, existing in json_file["missing_combinations"]:
                                if existing[-1] in global_counters and existing in global_counters:
                                    global_counters[non_existing] = (
                                        global_counters[existing[-1]] - global_counters[existing]
                                        if global_counters[existing[-1]] - global_counters[existing] > 0
                                        else 0
                                    )
                            sum_counters = None
                            if os.path.exists(f"{train_parameters.fed_dir}/sum_counters.pkl"):
                                with open(f"{train_parameters.fed_dir}/sum_counters.pkl", "rb") as file:
                                    sum_counters = dill.load(file)

                            # sum the values inside the two dictionaries
                            if sum_counters and global_counters:
                                for key in sum_counters.keys():
                                    if key in global_counters:
                                        sum_counters[key] += int(global_counters[key])

                                # compute the disparity considering the values inside sum_counters
                                print("Summing current local unfairness with global unfairness")
                                fairness_violation = 0
                                for target in [0, 1]:
                                    for sensitive_value in [0, 1]:
                                        current_violation = (
                                            (sum_counters[f"{target}|{sensitive_value}"])
                                            / (
                                                sum_counters[f"{target}|{sensitive_value}"]
                                                + sum_counters[f"{abs(1 - target)}|{sensitive_value}"]
                                            )
                                        ) - (
                                            (sum_counters[f"{target}|{abs(1 - sensitive_value)}"])
                                            / (
                                                sum_counters[f"{target}|{abs(1 - sensitive_value)}"]
                                                + sum_counters[f"{abs(1 - target)}|{abs(1 - sensitive_value)}"]
                                            )
                                        )
                                        fairness_violation = max(fairness_violation, current_violation)
                                fairness_violation = torch.tensor(fairness_violation)
                                fairness_violation_with_noise = fairness_violation.item()
                            else:
                                if sigma_update_lambda:
                                    noise = Utils.get_noise(
                                        mechanism_type="gaussian",
                            ...
                            if average_probabilities:
                                # in this case we loaded the average proba from the disk and we need to
                                # sum them to the counters computed for this client
                                if sigma_update_lambda:
                                    noise = Utils.get_noise(
                                        mechanism_type="gaussian",
                                        sigma=sigma_update_lambda,
                                ...
                                for key in average_probabilities.keys():
                                    if key in global_counters:
                                        global_counters[key] = (
                                            (average_probabilities[key] + noise) + global_counters[key]
                                        ) / 2
                                fairness_violations = []
                                for unprivileged in list(train_parameters.unprivileged_group):
                                    for privileged in list(train_parameters.privileged_group):
                                        error_rate = (
                                            global_counters[unprivileged] - global_counters[privileged]
                                            if global_counters[unprivileged] - global_counters[privileged] > 0
                                            else torch.tensor(0.0)
                                        )
                                        if not isinstance(error_rate, torch.Tensor):
                                            error_rate = torch.tensor(error_rate)
                                ...
                                fairness_violation = max(fairness_violations)
                                fairness_violation = torch.tensor(fairness_violation)
                                fairness_violation_with_noise = fairness_violation.item()
                                print(
                                    "Fairness Violations: ",
                    ...
                    history_disparity.append(fairness_violation.item())
                    ...
                    if fairness_violation_with_noise < 0:
                        fairness_violation_with_noise = 0
                    if fairness_violation_with_noise > 1:
                        fairness_violation_with_noise = 1
                    ...
                    new_lambda = train_parameters.regularization_lambda - velocity * alpha

                    print(
                        f"Old Lambda {train_parameters.regularization_lambda} - New Lambda {new_lambda} - Delta {delta} - Velocity {velocity} - Alpha {alpha} - Momentum {train_parameters.momentum}"
                    )
                    ...
                    train_parameters.velocity = velocity

                    if train_parameters.regularization_lambda == None:
                        print("Lambda is none ERROR")
        ...
        if train_parameters.weight_decay_alpha:
            alpha = exp_lr_scheduler(
                initial_alpha=alpha,
                current_fl_round=current_epoch,  # current_fl_round,
                decay_rate=train_parameters.weight_decay_alpha,
            )

        train_parameters.alpha = alpha
        train_loss_original = losses
        train_loss_with_regularization_original = losses_with_regularization
        ...
        if train_parameters.metric == "disparity":
            ...
            max_unfairness_train = criterion_regularization.violation_with_dataset(
                ...
                device=train_parameters.device,
                average_probabilities=average_probabilities,
                privileged_group=train_parameters.privileged_group,
                unprivileged_group=train_parameters.unprivileged_group,
            )
        ...
        return {
            ...
            "Train Loss_or": train_loss_original,
            "Train Loss + Regularizaion_or": train_loss_with_regularization_original,
            "Train Accuracy": accuracy,
            "Max Unfairness Train": max_unfairness_train,
            "history_lambda": history_lambda,
            "history_disparity": history_disparity,
    ...
    def test(
        ...
        elif train_parameters.metric == "equalised_odds":
    ...
    def compute_regularization_term(
        ...
        if train_parameters.metric == "disparity":
            if global_computation:
                fairness_violation, global_counters = criterion_regularization(
                ...
                return fairness_violation, global_counters
            else:
